...
map<Node, Point> NodePositioner::get_node_positions(Node root_node, double graph_scale) {
	...
	map<ogdf::node, Node> ogdf_to_node_map;
	...
	ga.setAllWidth(Image::THUMB_WIDTH * graph_scale);
	ga.setAllHeight(Image::THUMB_HEIGHT * graph_scale);
	...
	for (Node n : m_graph->get_nodes()) {
		...
		for (auto it = node_it.first; it != node_it.second; ++it) {
			...
			weight[e] = m_graph->get_edge_weight(node1, node2) * graph_scale;
		}
	}
